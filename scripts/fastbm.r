...
main <- function(opt){
    ...
    df_bm_simu <- as.data.frame(fastBM(tree=tree, a=as.double(opt$a), sig2=as.double(opt$sig2), bounds=c(as.double(opt$bound_inf),as.double(opt$bound_sup)), nsim=as.integer(opt$nsim))
